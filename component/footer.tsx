    ...
    <footer style={{ backgroundColor: BG }} className="text-white">
      <div className="mx-auto grid max-w-7xl gap-8 max-sm:gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[1.1fr_0.8fr_0.8fr_1.4fr] lg:px-8 xl:px-10">
        <div className="space-y-5">
          ...
          <BookingButton
            ...
            className="inline-flex rounded-tl-[25px] rounded-br-[25px] bg-[#C9A96E] px-6 py-2.5 text-sm font-semibold tracking-wide text-black transition-all duration-200 hover:border-white hover:bg-white hover:text-black active:scale-95"
          >
            Book Appointment
          </BookingButton>
        </div>
        ...
        <div>
          ...
          <ul className="mt-5 space-y-1">
            {contactDetails.map((detail) => (
              <li key={detail.label} className="text-sm leading-6 text-white/70">
      ...
      <div className="border-t border-white/10 max-sm:mb-15">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-2 text-sm text-white sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8 xl:px-10">
          ...
          <Link
            href="/privacy-policy"
            className="text-sm text-white transition-colors duration-200 hover:text-[#C9A96E]"
          >
            Privacy Policy
          </Link>
        </div>
